...
class DoubleCake:
    ...
    def __init__(self, num_samples, num_sectors):
        ...
        self.num_sectors = num_sectors
        if self.num_samples != 0:
            X1, Y1, labels1, labels_sym1 = self._make_data()
            X2, Y2, labels2, labels_sym2 = self._make_data()

            self.X = np.hstack([X1, .5 * X2])
            self.Y = np.hstack([Y1, .5 * Y2])
            self.labels = np.hstack([labels1, -labels2+1])
            self.labels_sym = np.hstack([labels_sym1, -1 * labels_sym2])
        else:
            X1, Y1, labels1, labels_sym1 = self._make_simple_data()
            X2, Y2, labels2, labels_sym2 = self._make_simple_data()
    ...
    def plot(self, ax):
        ...
        for i in range(self.num_sectors):
            ...
            ax.add_artist(Wedge((0, 0), 1, i * sector_angle, (i+1)*sector_angle, lw=0, color=color, alpha=0.1, width=.5))
            ax.add_artist(Wedge((0, 0), .5, i * sector_angle, (i+1)*sector_angle, lw=0, color=other_color, alpha=0.1))
            ax.set_xlim(-1, 1)
