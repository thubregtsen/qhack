...
## fit the SVM
svm_untrained_kernel = SVC(kernel=lambda X1, X2: k.kernel_matrix(X1, X2, init_params)).fit(X, y)
untrained_accuracy = accuracy(svm_untrained_kernel, X, y)
print(untrained_accuracy)
...
for i in range(500):
    ...
    if (i+1) % 50 == 0:
        print("Step {} - Alignment = {:.3f}".format(i+1, k.target_alignment(X, y, params)))

## fit the SVM
svm_trained_kernel = SVC(kernel=lambda X1, X2: k.kernel_matrix(X1, X2, init_params)).fit(X, y)
trained_accuracy = accuracy(svm_untrained_kernel, X, y)
