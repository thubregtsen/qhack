...
def target_alignment_grad(X, y, kernel, kernel_param, dx=1e-6):
    ...
    """
    model = np.copy(kernel_param)
    flat_param = np.array(list(_flatten(kernel_param)))
    g = np.zeros_like(flat_param)
    shifts = np.eye(len(flat_param))*dx/2
    for i, shift in enumerate(shifts):
        param_plus = unflatten((flat_param+shift), model)
        param_minus = unflatten((flat_param-shift), model)
        ta_plus = kernel.target_alignment(X, y, param_plus)
        ta_minus = kernel.target_alignment(X, y, param_minus)
