...
class Dataset:
    ...
    def __init__(self, num_samples_train, num_samples_test, noise=None):
        self.X_train, self.y_train = make_moons(num_samples_train, noise=noise)
        self.X_test, self.y_test = make_moons(num_samples_test, noise=noise)
    ...
    def plot(self, ax):
        ax.scatter(self.X_train[:,0], self.X_train[:,1], c=self.y_train, cmap=mpl.colors.ListedColormap(['#FF0000', '#0000FF']), s=25, marker='o')
        ax.scatter(self.X_test[:,0], self.X_test[:,1], c=self.y_test, cmap=mpl.colors.ListedColormap(['#FF0000', '#0000FF']), s=25, marker='x')
...
# +
dataset = Dataset(20, 20, noise=0.1)
